...
test "Binary Search" {
    ...
    var arr: std.ArrayList(u16) = try .initCapacity(u16);
    ...
    for (inputs, answers) |input, answer| {
        ...
        try std.testing.expect(answer.was_visited == new_search.was_visited);
        if (answer.was_visited == true) {
