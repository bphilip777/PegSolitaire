...
const getNumMoves = @import("Board.zig").getNumMoves;
...
// TODO: Context-Dependent MultiArrayList?
// - two types of structs:
// - 1. manual: internal multi array list
// - 2. auto: external multi array list to maximize reduction in memory footprint
...
fn dfs(allo: Allocator) !void {
    ...
    defer for (stack.items) |*board| board.deinit(allo);
    // start
    const start: Board = try .init(allo, 0);
    ...
    while (stack.items.len > 0) {
        ...
        const prev_board = if (rem_moves > 1) stack.getLast() else stack.pop().?; // pop needs to be deinit
        const search = binarySearch(&visited, &prev_board);
        if (search.visited) {
            var board = visited.items[search.idx];
            const move = findMove(&board);
            board.chooseMove(move.idx, move.dir);
        } else {
            var board = prev_board;
            const move = findMove(&board);
            board.chooseMove(move.idx, move.dir);
        }
    }
...
fn findMove(board: *const Board) struct { idx: u16, dir: Direction } {
    std.debug.assert(getAllMoves(&board.moves) > 0);
    for (board.moves, 0..) |move, i| {
        for ([_]Direction{ .Left, .UpLeft, .UpRight, .Right, .DownRight, .DownLeft }) |dir| {
            if (move.contains(dir)) return .{ @truncate(i), dir };
        }
    } else unreachable;
}
...
fn binarySearch2(boards: *const std.ArrayList(Board), board: *const Board) Search {
